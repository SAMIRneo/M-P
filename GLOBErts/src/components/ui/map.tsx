...
function Map({ children, styles, ...props }: MapProps) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => setMapInstance(mapInstance));
  ...
  useEffect(() => {
    if (mapInstance) {
      queueMicrotask(() => setIsStyleLoaded(false));
...
function MarkerPopup({
  ...
  useEffect(() => {
    ...
    const container = document.createElement("div");
    queueMicrotask(() => setContainerEl(container));
...
function MarkerTooltip({
  ...
  useEffect(() => {
    if (!isReady || !markerRef.current || !markerElement || !map) return;
  ...
  }, [isReady, map, markerElement, markerRef]);
...
function MapPopup({
  ...
  useEffect(() => {
    ...
    const popup = new MapLibreGL.Popup({
      ...
      .setMaxWidth("none")
      .setDOMContent(container)
      .setLngLat([lngLatRef.current.longitude, lngLatRef.current.latitude])
      .addTo(map);
...
function MapRoute({
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      paint: {
        "line-color": initialPaintRef.current.color,
        "line-width": initialPaintRef.current.width,
        "line-opacity": initialPaintRef.current.opacity,
        ...(initialPaintRef.current.dashArray && {
          "line-dasharray": initialPaintRef.current.dashArray,
        }),
      },
    });
...
>({
  ...
  const initialClusterOptionsRef = useRef({ clusterMaxZoom, clusterRadius });
  const initialStyleRef = useRef({ clusterColors, clusterThresholds, pointColor });
  ...
  useEffect(() => {
    ...
    map.addSource(sourceId, {
      ...
      cluster: true,
      clusterMaxZoom: initialClusterOptionsRef.current.clusterMaxZoom,
      clusterRadius: initialClusterOptionsRef.current.clusterRadius,
    });
    ...
    map.addLayer({
      ...
      paint: {
        "circle-color": [
          "step",
          ["get", "point_count"],
          initialStyleRef.current.clusterColors[0],
          initialStyleRef.current.clusterThresholds[0],
          initialStyleRef.current.clusterColors[1],
          initialStyleRef.current.clusterThresholds[1],
          initialStyleRef.current.clusterColors[2],
        ],
        "circle-radius": [
          "step",
          ["get", "point_count"],
    ...
    map.addLayer({
      ...
      filter: ["!", ["has", "point_count"]],
      paint: {
        "circle-color": initialStyleRef.current.pointColor,
        "circle-radius": 6,
      },
    });
