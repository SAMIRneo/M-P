...
export class Volcano {
  ...
  private smokeMat: THREE.MeshToonMaterial;
  ...
  private createLava() {
    ...
    this.lavaLight = new THREE.PointLight(0xff4400, 1000, 300);
    this.lavaLight.position.set(0, 20, 0);
  ...
  public update(time: number, delta: number) {
    ...
    const lavaFlash = Math.sin(time * 15) * 0.1;
    
    if (this.lava) {
      this.lava.scale.setScalar(0.85 + lavaPulse * 0.2 + lavaFlash);
      (this.lava.material as THREE.MeshBasicMaterial).color.setHex(lavaPulse > 0.8 ? 0xff4400 : 0xee2200);
    }
